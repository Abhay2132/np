const styles = {
    ...
    bgTxt : "position: relative ;text-align: center; box-sizing: border-box; border-radius: 0px;height: 28px;background: rgba(0,0,0,0.2); width: 298px; padding: 7px; font-size: 14px; color : #162;",
	...
	glare : "height: 28px; position: relative ; width: 80px; background: linear-gradient(90deg, rgba(250,250,250,0), rgba(250,250,250,0.5), rgba(250,250,250,0.8)); animation: glare 2s linear 0s infinite normal ; top: -28px; border-radius: 0px 3px 3px 0px;"
},
	kf = `
@keyframes glare {
    from {left: -80px;}
...
window.progressBar = function ( pTag ) {
	...
	let ac = ( a , ...c ) => c.forEach( b =>  a.appendChild(b)),
		d = (s) => { let tag = document.createElement("div") ; tag.style = s; return tag; },
		s = (h) => { let tag = document.createElement("style") ; tag.innerHTML = h; return tag; }
	for(let t in styles ) me[t] = d( styles[t])
	
	this.init = pTag => ac(pTag, me.pg, s(kf))
	
	this.config = pgBody => {
		ac(me.bar, me.txt); ac(me.bar, me.glare);
		ac(me.pg, me.bgTxt) ; ac(me.pg, me.bar);
		me.text = "" ; me.width = "1%"
	}
	this.config()
